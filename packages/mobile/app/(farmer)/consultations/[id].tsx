...
export default function FarmerChatScreen() {
    ...
    const onVideoCallStarted = ({ callerUserId, callerName }: { callerUserId: string; callerName: string; roomName: string }) => {
      ...
      Alert.alert(
        ...
        [
          ...
          { text: 'Rejoindre', onPress: () => router.push(`/(farmer)/consultations/video/${id}` as any) },
        ]
      );
    };
  ...
  const joinVideoCall = () => {
    router.push(`/(farmer)/consultations/video/${id}` as any);
  };

  const statusCfg = statusConfig[(consultation?.status as keyof typeof statusConfig) ?? 'PENDING'];
  const vetName   = consultation?.vet?.user?.fullName ?? 'Vétérinaire';
      ...
      <View style={[styles.msgRow, isMine ? styles.msgRowMe : styles.msgRowOther]}>
        {!isMine && (
          <View style={styles.otherAvatar}>
            <Text style={styles.otherAvatarText}>{vetName.charAt(0)}</Text>
          </View>
        )}
  ...
  return (
    <View style={styles.root}>
      <LinearGradient colors={['#011C12', '#022C22', '#065F46']} style={styles.header}>
          ...
          <View style={styles.headerInner}>
            ...
            <View style={styles.headerCenter}>
              <View style={styles.vetAvatar}>
                <Text style={styles.vetAvatarText}>{vetName.charAt(0)}</Text>
              </View>
              <View>
                <Text style={styles.vetName} numberOfLines={1}>{vetName}</Text>
            ...
            {/* Video call button — visible when active */}
            {isActive && (
      ...
      {/* Video call banner for VIDEO type consultations */}
      {isActive && isVideo && (
      ...
      {consultation?.symptomsDescription && (
        <View style={styles.symptomsBanner}>
          <Ionicons name="document-text-outline" size={14} color={colors.amber[600]} />
          <Text style={styles.symptomsText} numberOfLines={2}>{consultation.symptomsDescription}</Text>
        </View>
      )}
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyChat}>
                ...
                <Text style={styles.emptyChatText}>Démarrez la conversation !{'\n'}Décrivez vos préoccupations au vétérinaire.</Text>
              </View>
            }
          />
        )}
...
const styles = StyleSheet.create({
  ...
  vetAvatarText: { fontSize: 16, fontWeight: '700', color: '#fff' },
  vetName: { fontSize: 15, fontWeight: '700', color: '#fff' },
  ...
  symptomsBanner: {
    ...
    backgroundColor: colors.amber[50], borderRadius: radius.lg,
    padding: spacing[3], borderWidth: 1, borderColor: colors.amber[100],
  },
  ...
  sendBtn: {
    ...
    backgroundColor: colors.brand[600], alignItems: 'center', justifyContent: 'center', ...shadow.md,
  },
